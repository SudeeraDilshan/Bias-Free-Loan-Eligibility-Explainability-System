...
class DataPreprocessor:
    ...
    def load_data(self, filepath: str) -> pd.DataFrame:
        ...
        if not os.path.exists(filepath):
            ...
            fallback = os.path.join(dir_name if dir_name else '.', 'sri_lanka_loan_data.csv')
            if os.path.exists(fallback):
                print(f"⚠️ Warning: File '{filepath}' not found. Falling back to '{fallback}'")
                filepath = fallback
            else:
                fallback_alt = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'data', 'sri_lanka_loan_data.csv')
                if os.path.exists(fallback_alt):
                    print(f"⚠️ Warning: File '{filepath}' not found. Falling back to '{fallback_alt}'")
                    filepath = fallback_alt
    ...
    def prepare_data(self, filepath: str, test_size: float = 0.2, 
        ...
        # Preserve the exact feature order used for training
        feature_order = list(X.columns)
        self.preprocessor_config['feature_order'] = feature_order
